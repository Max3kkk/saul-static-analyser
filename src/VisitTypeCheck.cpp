...
{ 
            ...
            superTypeStr.size() > 11 && superTypeStr.substr(0, 11) == "(TypeRecord")
        {
            ...
            for (ListRecordFieldType::iterator i = superTypeRecord->listrecordfieldtype_->begin(); i != superTypeRecord->listrecordfieldtype_->end(); ++i)
            {
                ...
                for (ListRecordFieldType::iterator j = subTypeRecord->listrecordfieldtype_->begin(); j != subTypeRecord->listrecordfieldtype_->end(); ++j)
                {
                    ...
                    if (AFTj->saulident_ == AFTi->saulident_ && compareTypes(AFTj->type_, AFTi->type_))
                        found = true;

                    if (AFTj->saulident_ == AFTi->saulident_)
                        entered = true;
                }
    ...
    void VisitTypeCheck::visitAParamDecl(AParamDecl *a_param_decl)
    {
        ...
        this->addVariableDecl(a_param_decl->saulident_, type_Type, 1);
    ...
    void VisitTypeCheck::visitTypeVar(TypeVar *type_var)
    {
        ...
        ifDefined(type_var->saulident_, type_var->line_number);

        visitSaulIdent(type_var->saulident_);

        // Return variable type
        this->setLastReturn(this->declearedVariables[type_var->saulident_].type);
    }
    ...
    void VisitTypeCheck::visitPatternVar(PatternVar *pattern_var)
    {
        ...
        if (this->declearedVariables.count(pattern_var->saulident_) == 0)
        {
            this->addVariableDecl(pattern_var->saulident_, input, 0);
        }
        else
        {
            if (compareTypes(this->declearedVariables[pattern_var->saulident_].type, input) == false)
            {
                std::cout << "Error: incorrect parameters in the match" << std::endl;
                std::cout << "Expected: " << showner->show(input) << std::endl
                          << "But got: " << showner->show(this->declearedVariables[pattern_var->saulident_].type) << std::endl;
                exit(1);
            }
        }
    }
    ...
    void VisitTypeCheck::visitDotRecord(DotRecord *dot_record)
    {
        ...
        for (ListRecordFieldType::iterator i = returnRecord->listrecordfieldtype_->begin(); i != returnRecord->listrecordfieldtype_->end(); ++i)
        {
            ...
            if (ARFT->saulident_ == dot_record->saulident_)
            {
    ...
    void VisitTypeCheck::visitRecord(Record *record)
    {
        ...
        for (ListBinding::iterator i = record->listbinding_->end() - 1; i != record->listbinding_->begin() - 1; --i, --j)
        {
            ...
            returnTypeList = consListRecordFieldType(new ARecordFieldType(AB->saulident_, listbindingCopy[j]), returnTypeList);
        }
    ...
    void VisitTypeCheck::visitPred(Pred *pred)
    {
        ...
        // Check if succ parameter is correctly handled
        nullptrCheck(expr_Type, pred->expr_->line_number, pred->expr_->char_number);

        // Check if succ parameter is correctly typed
        if (compareTypes(expr_Type, new TypeNat()) == false)
        {
            std::cout << "Error: incorrect type of succ argument: " << getNodeID(pred->expr_) << std::endl;
            exit(1);
        }
    ...
    void VisitTypeCheck::visitIsZero(IsZero *is_zero)
    {
        ...
        if (compareTypes(expr_Type, new TypeNat()) == false)
        {
            std::cout << "Error: incorrect type of succ argument: " << getNodeID(is_zero->expr_) << std::endl;
            exit(1);
        }
    ...
    void VisitTypeCheck::visitConstInt(ConstInt *const_int)
    {
        ...
        {
            std::cout << "Error: invalid constant integer. In the Saul languagne only 0 is valid constInt" << std::endl
    ...
    void VisitTypeCheck::visitVar(Var *var)
    {
        ...
        ifDefined(var->saulident_, var->line_number);

        visitSaulIdent(var->saulident_);

        // Return variable type
        this->setLastReturn(this->declearedVariables[var->saulident_].type);
    }
    ...
    void VisitTypeCheck::visitListSaulIdent(ListSaulIdent *list_saul_ident)
    {
    ...
    void VisitTypeCheck::visitListLabelledPattern(ListLabelledPattern *list_labelled_pattern)
    {
             ...
             i != list_labelled_pattern->end() && j != input->listrecordfieldtype_->end(); ++i, ++j)
        {
            ...
            if (ARFT->saulident_ != ALP->saulident_)
            {
                std::cout << "Error: incorrect name in the pattern record" << std::endl
                          << "Expected: " << ARFT->saulident_ << std::endl
                          << "But got: " << ALP->saulident_ << std::endl;
            }
    ...
    void VisitTypeCheck::visitSaulIdent(SaulIdent x)
    {
